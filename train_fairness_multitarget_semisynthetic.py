...
parser.add_argument('b', help='offset for the gagne_sum_t synthetic outcome [float from -1.0 to 1.0 by steps of 0.1]')
...
# extract the b_value from the arguments to a variable called b_value
b_value = args.b

# update settings['data_name'] to include the b_value
settings['data_name'] = settings['data_name'] + '{}'.format(b_value)

# extract just the filename of settings['data_name'] to a variable called data_str
data_str = settings['data_name'].split('/')[-1]
...
if (c.shape[1] >30):
    ...
    all_dropped = c[["index", 'dem_race_black', "split"]]
...
y_multi = training_df[['log_cost_t', 'gagne_sum_t']]
...
training_ranks_df =pd.DataFrame(np.array(y_rank_multi).T, columns= ['y1', 'y2'])
...
for index, row in training_ranks_df.iterrows():
    ...
    these_ranks = np.array([row['y1'], row['y2']])
...
training_y_pred_df = pd.DataFrame(np.array(y_pred_multi).T, columns= ['y1', 'y2'])
...
## for running semi-synthetic data
stand_y_pred_df = pd.DataFrame({'y1': standardize_y(training_y_pred_df['y1']),
                                 'y2': standardize_y(training_y_pred_df['y2'])})
...
y_multi_tune = tune_alphas_df[['log_cost_t', 'gagne_sum_t']]
...
y_pred_multi_tune = pd.DataFrame(np.array(y_hat_tune).T, columns= ['y1', 'y2'])
...
## for running semi-synthetic version
stand_y_pred_tune_df = pd.DataFrame({'y1': standardize_y(y_pred_multi_tune['y1']),
                                 'y2': standardize_y(y_pred_multi_tune['y2'])})
...
tune_ranks_df = pd.DataFrame(np.array(tune_r).T, columns= ['y1', 'y2'])
...
model_max.addConstr(sum(alpha[target] for target in range(n_targets)) >= 0.5, name='alpha_not_0')
...
for aa in range(n_groupA):
    ...
    for bb in range(samples_tune):
        if (bb != aa):
            ...
            # # Big-M constraint
            model_max.addConstr((this_index_model_bb - this_index_model_aa) - big_M * I_ba[bb, aa] <= 0.0,
                                       name="M1_{},{}".format(bb, aa))

            # model_max.addConstr((this_index_model_aa - this_index_model_bb) - big_M * I_ab[aa, bb] <= 0.0,
...
#todo: fix the way T is unpacked from gurobi
# T_lst = []
# for tt in model_max.getVars()[n_targets: n_targets + n_groupA]:
#     item = {tt.VarName: tt.X}
#     t_value = tt.X
#     T_lst.append(tt.X)
#
# T_var_result = np.array(T_lst)
#
# obj_output = np.sum([T_var_result[ex]* race_black_indicator[ex]  for ex in range(n_groupA)])
#
# print_log('***** Max top-K selection for group A *****')
# print_log('total sum group A: {}, top_K_tune: {}, Obj: {}'.format(np.sum(race_black_indicator), top_K_tune, obj_output))
...
y_multi_holdout = holdout_df[['log_cost_t', 'gagne_sum_t']]
...
y_pred_multi_holdout = pd.DataFrame(np.array(y_hat_holdout).T, columns= ['y1', 'y2'])

rank_multi_holdout = pd.DataFrame(np.array(rank_holdout).T, columns= ['y1', 'y2'])
...
# new_df.append({'n_y1': y_hat_holdout_top_K['y1'],
#                'n_y2': y_hat_holdout_top_K['y2'],
#                'n_y3': y_hat_holdout_top_K['y3'],
#                'n_index': index_n_groupA})

# test_alphas = np.array([0.5, 0.0, 0.5])
# index_hold_TEST = stand_y_pred_holdout_df.dot(test_alphas)
# index_rank_TEST = pd.DataFrame(rankdata(-1.0 * index_hold_TEST, method=rank_method))
# # we want to know how many Black patients are ranked in top_K with index model
# idx_n_TEST = (index_rank_TEST <= top_K_holdout).iloc[np.array(race_black_holdout) == 1].sum(axis = 0)
...
dropped_hold_out['gagne_sum_t_hat'] = y_hat_holdout[1]
...
## for running semi-synthetic version
stand_y_true_holdout_df = pd.DataFrame({'y1': standardize_y(holdout_df['log_cost_t']),
                                 'y2': standardize_y(holdout_df['gagne_sum_t'])})
...
dropped_hold_out['alpha_gagne_sum_t'] = alphas_array[1]





semi_synthetic = {'b': b_value,
                  'data': 'hold_out',
                  'top_K': top_K_holdout,
                  'frac_log_cost_t': y_hat_holdout_top_K[0] / top_K_holdout,
                  'frac_gagne_t': y_hat_holdout_top_K[1] / top_K_holdout,
                  'alpha_log_cost_t': alphas_array[0],
                  'alpha_gagne_t': alphas_array[1],
                  'frac_index_model': index_n_groupA / top_K_holdout
                  }

# save results in a csv that is named to include the tune top_K and tune samples
pd.DataFrame(semi_synthetic, index=[0]).to_csv('results/{}_alphas_tuneK{}_tuneN{}.csv'.format(data_str, top_K_tune, samples_tune))

dropped_hold_out.to_csv('results/{}_table_results_tuneK{}_tuneN{}.csv'.format(data_str, top_K_tune, samples_tune))

marker_sz = 3
# plotting for testing semi-synthetic results
plt.figure()
plt.scatter(holdout_df['dem_age'], stand_y_true_holdout_df['y1'], label='log_cost_t', s=marker_sz )
plt.scatter(holdout_df['dem_age'], stand_y_true_holdout_df['y2'], label='gagne_sum_t', s=marker_sz )
# plt.scatter(holdout_df['dem_age'], stand_y_true_holdout_df['y3'], label='gagne_sum_t', s=marker_sz)
plt.scatter(holdout_df['dem_age'], np.array(index_model_holdout), label='index', s=marker_sz )
plt.legend()
...
plt.figure()
plt.scatter(tune_alphas_df['dem_age'], stand_y_pred_tune_df['y1'], label='log_cost_t', s=marker_sz )
plt.scatter(tune_alphas_df['dem_age'], stand_y_pred_tune_df['y2'], label='gagne_sum_t', s=marker_sz )
# plt.scatter(tune_alphas_df['dem_age'], stand_y_pred_tune_df['y3'], label='gagne_sum_t', s=marker_sz)
plt.scatter(tune_alphas_df['dem_age'], np.array(index_tune_check), label='index', s=marker_sz )
plt.legend()
plt.title('Tune data')
plt.axvline(x = 45, color = 'b')
plt.axvline(x = 54, color = 'b')
